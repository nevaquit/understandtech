---
name: garrytan-gstack-design-review
preamble-tier: 4
version: 2.0.0
...
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(.cursor/skills/gstack/bin/gstack-config get skill_prefix 2>/dev/null || echo "false")
...
_SESSION_KIND=$(.cursor/skills/gstack/bin/gstack-session-kind 2>/dev/null || echo "interactive")
...
_EXPLAIN_LEVEL=$(.cursor/skills/gstack/bin/gstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(.cursor/skills/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x ".cursor/skills/gstack/bin/gstack-telemetry-log" ]; then
      .cursor/skills/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
if [ -f "$_LEARN_FILE" ]; then
  ...
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    .cursor/skills/gstack/bin/gstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
.cursor/skills/gstack/bin/gstack-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(.cursor/skills/gstack/bin/gstack-config get routing_declined 2>/dev/null || echo "false")
...
_CHECKPOINT_MODE=$(.cursor/skills/gstack/bin/gstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(.cursor/skills/gstack/bin/gstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `.cursor/skills/gstack/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `.cursor/skills/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running gstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `.cursor/skills/gstack/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `.cursor/skills/gstack/bin/gstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `.cursor/skills/gstack/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `.cursor/skills/gstack/bin/gstack-config set explain_level terse`.
...
If A: run `.cursor/skills/gstack/bin/gstack-config set proactive true`
If B: run `.cursor/skills/gstack/bin/gstack-config set proactive false`
...
If B: run `.cursor/skills/gstack/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.
...
3. Run `.cursor/skills/gstack/bin/gstack-team-init required` (or `optional`)
...
_BRAIN_SYNC_BIN=".cursor/skills/gstack/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN=".cursor/skills/gstack/bin/gstack-config"
...
```bash
".cursor/skills/gstack/bin/gstack-brain-sync" --discover-new 2>/dev/null || true
".cursor/skills/gstack/bin/gstack-brain-sync" --once 2>/dev/null || true
```
...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    .cursor/skills/gstack/bin/gstack-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
**Cross-session decisions.** If `ACTIVE DECISIONS` are listed, treat them as prior settled calls with their rationale — do not silently re-litigate them; if you're about to reverse one, say so explicitly. Reach for `.cursor/skills/gstack/bin/gstack-decision-search` whenever a question touches a past decision ("what did we decide / why / did we try"). When you or the user make a DURABLE decision (architecture, scope, tool/vendor choice, or a reversal) — NOT a turn-level or trivial choice — log it with `.cursor/skills/gstack/bin/gstack-decision-log` (`--supersede <id>` for a reversal). Reliable and local; gbrain not required.
...
Curated jargon list lives at `.cursor/skills/gstack/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `.cursor/skills/gstack/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
...
```bash
.cursor/skills/gstack/bin/gstack-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
.cursor/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `.cursor/skills/gstack/ETHOS.md`.
...
```bash
.cursor/skills/gstack/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
[ -z "$D" ] && D=".cursor/skills/gstack/design/dist/design"
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
```bash
_CROSS_PROJ=$(.cursor/skills/gstack/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  .cursor/skills/gstack/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  .cursor/skills/gstack/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `.cursor/skills/gstack/bin/gstack-config set cross_project_learnings true`
If B: run `.cursor/skills/gstack/bin/gstack-config set cross_project_learnings false`
...
**Log the result:**
```bash
.cursor/skills/gstack/bin/gstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
.cursor/skills/gstack/bin/gstack-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
