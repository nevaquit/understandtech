...
Provides commands to clear and inspect caches used by the cursor-docs plugin.
...
def cmd_clear_all(cm: CacheManager) -> int:
    ...
    if not cleared_any:
        print("No caches to clear (already empty)")
    else:
...
def cmd_clear_inverted(cm: CacheManager) -> int:
    ...
    if cm.clear_inverted_index():
        ...
        print("Inverted index cache was already empty")
    return 0
...
def cmd_clear_llms(cm: CacheManager) -> int:
    ...
    if cm.clear_llms_cache():
        ...
        print("LLMS cache was already empty")
    return 0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Cache management for cursor-docs plugin',
