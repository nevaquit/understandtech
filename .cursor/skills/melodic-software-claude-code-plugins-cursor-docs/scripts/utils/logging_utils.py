...
# -*- coding: utf-8 -*-
"""
logging_utils.py - Common logging and observability utilities for cursor-docs scripts

Adapted for Cursor IDE and CLI Documentation from openai-ecosystem codex-cli-docs skill.

Provides structured logging, performance tracking, and observability features
for all scripts in the cursor-docs skill.
...
RUN_ID_ENV_VAR = "CURSOR_DOCS_RUN_ID"
SOURCE_NAME_ENV_VAR = "CURSOR_DOCS_SOURCE_NAME"
...
# CURSOR_DOCS_LOG_TO_FILE: "true" to enable file logging
# CURSOR_DOCS_LOG_LEVEL: DEBUG, INFO, WARNING, ERROR
# CURSOR_DOCS_LOG_JSON: "true" for JSON format
...
def _get_log_level() -> int:
    ...
    # Check env var first
    env_level = os.environ.get("CURSOR_DOCS_LOG_LEVEL", "").upper()
    if env_level:
...
class ScriptLogger:
    ...
    def __init__(
        ...
        self._enable_file_logging = (
            ...
            else _get_config_bool("logging", "enable_file_logging", "CURSOR_DOCS_LOG_TO_FILE", False)
        )
        self._json_format = (
            json_format if json_format is not None
            else _get_config_bool("logging", "json_format", "CURSOR_DOCS_LOG_JSON", False)
        )
        self._write_diagnostics = _get_runtime_value("diagnostics", "write_diagnostics_files", False)
        # HTTP timing: env var CURSOR_DOCS_HTTP_TIMING overrides config (for --with-timing flag)
        self._track_http = _get_config_bool("diagnostics", "track_http_timings", "CURSOR_DOCS_HTTP_TIMING", True)
    ...
    def _add_file_handler(self, log_level: int) -> None:
        ...
        When CURSOR_DOCS_SOURCE_NAME is set (by parallel workers), creates
    ...
    def log_warning(self, message: str, **kwargs):
        ...
        context = ', '.join(f"{k}={v}" for k, v in kwargs.items())
        self.logger.warning(f"{message}" + (f" ({context})" if context else ""))
...
def get_logger(script_name: str | None = None) -> ScriptLogger:
    ...
    if _global_logger is None:
        if script_name:
            ...
            # Default logger
            _global_logger = ScriptLogger('cursor-docs')
