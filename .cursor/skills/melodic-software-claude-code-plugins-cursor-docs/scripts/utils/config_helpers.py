...
def get_http_user_agent() -> str:
    ...
    return get_default('http', 'user_agent', "Cursor-Docs-Scraper/1.0 (Educational purposes)")

def get_management_user_agent() -> str:
    """Get user agent string for management/cleanup operations."""
    return get_default('user_agents', 'management', "Cursor-Docs-Management-Bot/1.0")

def get_scraper_user_agent() -> str:
    """Get user agent string for scraping operations."""
    return get_default('user_agents', 'scraper', "Cursor-Docs-Scraper/1.0 (Educational purposes)")
...
def get_domain_weight(domain: str) -> float:
    ...
    Args:
        domain: Domain name (e.g., "cursor.com")
    ...
    # Try to match by category path (e.g., cursor.com/docs)
...
def _get_filtering_config() -> dict:
    ...
    if _filtering_cache is None:
        try:
            ...
            # Fallback: direct load from config dir
            try:
                import yaml
                from utils.common_paths import get_config_dir
                config_path = get_config_dir() / "filtering.yaml"
...
def get_query_stop_words() -> set[str]:
    ...
    Example: "when to use cursor" -> search for "cursor" only
...
def get_domain_stop_words() -> set[str]:
    ...
    be filtered from searches (e.g., cursor).
