...
# -*- coding: utf-8 -*-
"""
dev_mode.py - Development mode detection and path override for cursor-docs plugin.

This module provides explicit dev/prod mode separation via environment variable.
When CURSOR_DOCS_DEV_ROOT is set to a valid skill directory, all paths resolve
there instead of the installed plugin location.

Mode Detection Priority:
1. Dev Mode: CURSOR_DOCS_DEV_ROOT env var set to valid skill directory
...
Environment Variables:
    CURSOR_DOCS_DEV_ROOT: Path to development skill directory.
...
DEV_ROOT_ENV_VAR = "CURSOR_DOCS_DEV_ROOT"
...
def is_dev_mode() -> bool:
    ...
    Returns True if CURSOR_DOCS_DEV_ROOT is set to a valid skill directory.
...
def get_effective_skill_dir(fallback: Path) -> Path:
    ...
    If CURSOR_DOCS_DEV_ROOT is set and valid, returns that path.
