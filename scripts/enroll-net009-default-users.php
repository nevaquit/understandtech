<?php
/**
 * Enrol default users into NET009 (manual enrolment, student role).
 *
 * Idempotent — safe to re-run after seed, DB recovery, or deploy.
 * Run on VM: sudo -u www-data php /opt/understandtech-plugins/scripts/enroll-net009-default-users.php
 *
 * Optional env:
 *   NET009_ENROL_USERNAMES=admin,e2etest,nevaquit
 *
 ...
 * @return void
 */
function net009_ensure_manual_enrol_enabled(): void {
 ...
 * @return bool
 */
function net009_user_can_view_course(stdClass $course, stdClass $user): bool {
    return can_access_course($course, $user);
}

$defaultnames = 'admin,e2etest,nevaquit';
$usernamesraw = getenv('NET009_ENROL_USERNAMES') ?: $defaultnames;
...
echo "=== enroll NET009 course={$courseid} ===\n";
...
net009_ensure_manual_enrol_enabled();
...
foreach ($verifiedusers as $user) {
    if (!net009_user_can_view_course($course, $user)) {
        echo "capability_missing username={$user->username} id={$user->id}\n";
        $failed++;
    } else {
