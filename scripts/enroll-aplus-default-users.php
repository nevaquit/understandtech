<?php
/**
 * Enrol default users into APLUS (manual enrolment, student role).
 *
 * Idempotent — safe to re-run after seed, DB recovery, or deploy.
 * Run on VM: sudo -u www-data php /opt/understandtech-plugins/scripts/enroll-aplus-default-users.php
 *
 * Optional env:
 *   APLUS_ENROL_USERNAMES=admin,e2etest,nevaquit
 *
 ...
 * @return void
 */
function aplus_ensure_manual_enrol_enabled(): void {
 ...
 * @return bool
 */
function aplus_user_can_view_course(stdClass $course, stdClass $user): bool {
    return can_access_course($course, $user);
}

$defaultnames = 'admin,e2etest,nevaquit';
$usernamesraw = getenv('APLUS_ENROL_USERNAMES') ?: $defaultnames;
...
echo "=== enroll APLUS course={$courseid} ===\n";
...
aplus_ensure_manual_enrol_enabled();
